...
function getTaskFolder(folderName, StoreName, TaskList) {
    if (folderName === undefined || folderName === '') {
        // if folder path is not defined, return main Tasks folder
        var folder = outlookNS.Session.Stores.Item(StoreName).GetRootFolder().Folders.Item(TaskList)

    } else {
        // if folder path is defined then find it, create it if it doesn't exist yet
        try {
            var folder = outlookNS.Session.Stores.Item(StoreName).GetRootFolder().Folders.Item(TaskList).Folders(folderName);
        }
        catch (e) {
            outlookNS.Session.Stores.Item(StoreName).GetRootFolder().Folders.Item(TaskList).Add(folderName);
            var folder = outlookNS.Session.Stores.Item(StoreName).GetRootFolder().Folders.Item(TaskList).Folders(folderName);
        }
    }
    return folder;
...
function getTaskItems(folderName, StoreName, TaskList) {
    return getTaskFolder(folderName, StoreName, TaskList).Items;
}
